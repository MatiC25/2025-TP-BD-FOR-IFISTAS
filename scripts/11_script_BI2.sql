...
GO
CREATE OR ALTER PROCEDURE FORIF_ISTAS.Migracion_DimTiempo
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SELECT DISTINCT YEAR(fact_fecha_hora)
    FROM FORIF_ISTAS.Factura
    WHERE fact_fecha_hora IS NOT NULL

    UNION
    SELECT DISTINCT YEAR(pedi_fecha_hora)
    ...
    UNION
    SELECT DISTINCT YEAR(envi_fecha_entrega)
    FROM FORIF_ISTAS.Envio
    WHERE envi_fecha_entrega IS NOT NULL

    UNION
    SELECT DISTINCT YEAR(comp_fecha)
    FROM FORIF_ISTAS.Compra
    WHERE comp_fecha IS NOT NULL;
    DECLARE @anio INT;

    DECLARE anios_cursor CURSOR FOR SELECT anio FROM @anios;
    OPEN anios_cursor;
    FETCH NEXT FROM anios_cursor INTO @anio;
